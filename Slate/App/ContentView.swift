...
struct ContentView: View {
    ...
    private func parseResponse(_ response: String) -> (title: String, desc: String) {
        ...
        if cleanResponse.hasPrefix("```markdown") {
            cleanResponse = String(cleanResponse.dropFirst("```markdown".count)).trimmingCharacters(in: .whitespacesAndNewlines)
        } else if cleanResponse.hasPrefix("```") {
            cleanResponse = String(cleanResponse.dropFirst(3)).trimmingCharacters(in: .whitespacesAndNewlines)
        }
        ...
        for line in lines {
            ...
            // Skip internal triple backtick lines
